...
for (const question of questions) {
  ...
  // 使用检索器retriever先把问题向量化，检索最相关的3条文档
  const retrievedDocs = await retriever.invoke(question);
  // 检索最相关的3条文档并计算它们的相似度评分
  ...
  const context = retrievedDocs
    .map((doc, i) => `[片段${i + 1}]\n${doc.pageContent}`)
    .join("\n\n━━━━━\n\n");
  const propmt = `你是一个讲友情故事的老师。基于以下故事片段回答问题，用温暖生动的语言。如果故事中没有提到，就说"这个故事里还没有提到这个细节"。
...
老师的回答:`;
  ...
  // 调用语言模型生成答案
  console.log("\n【AI 回答】");
  const response = await model.invoke(propmt);
